...
async function importFixed() {
  console.log('🔄 Importing with password column fix...\n');
  ...
  try {
    ...
    // 4. Insert users with both password_hash and password
    let inserted = 0;
    for (const user of users.rows) {
      try {
        ...
        `, [
          nextId++, user.email, user.password_hash, tenantId, user.verified || true,
    ...
    const adminPasswordHash = '$2b$10$H7PzLp3qw5M5QFZ3m5Q5ZO5Z3m5Q5Z3m5Q5ZO5Z3m5Q5Z3m5Q5ZO5Z';
    ...
    if (adminCheck.rows.length === 0) {
      ...
      await targetPool.query(`
        ...
        ON CONFLICT (email) DO NOTHING
      `, [nextId++, adminPasswordHash, tenantId]);
