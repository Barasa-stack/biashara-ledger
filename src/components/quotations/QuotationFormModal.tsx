...
import { fetchNextQuotationNumber, saveQuotation } from '@/lib/api/quotations';
import { Customer, LineItem, QuotationForm, Quotation, emptyForm, STATUSES, PAYMENT_TERMS } from '@/types/quotations';
...
export function QuotationFormModal({ open, editing, customers, vatRate, onClose, onSaved, showToast }: Props) {
  ...
  useEffect(() => {
    ...
    if (editing) {
      let items: LineItem[] = [];
      try {
        const parsed = typeof (editing as any).items === 'string' ? JSON.parse((editing as any).items) : (editing as any).items;
      ...
      setLineItems([]);
      setForm(recalc({ ...emptyForm, issue_date: today, due_date: due }));
      fetchNextQuotationNumber().then(nextNumber => {
        setForm(prev => recalc({ ...prev, quotation_number: nextNumber }));
      });
    }
  }, [open]);

  const recalc = (f: QuotationForm, items?: LineItem[]) => {
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      await saveQuotation(form, lineItems, editing);
      onClose();
      onSaved();
      showToast(editing ? 'Quotation updated' : 'Quotation created', 'success');
    } catch (e: any) {
