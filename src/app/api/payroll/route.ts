  ...
  try {
    ...
    if (type === 'employee') {
      ...
      const result = await insertReturning<{ id: number }>(
        `INSERT INTO employees (employee_code, name, date_of_birth, national_id, tax_pin, phone, email, address, department, job_title, date_of_hire, employment_type, bank_name, account_number, emergency_contact_name, emergency_contact_phone, notes, salary, salary_encrypted, national_id_encrypted, bank_account_encrypted)
