  ...
  try {
    ...
    const result = await withTenantContext(session.tenant_id!, async () => {
      return await insertReturning<{ id: string }>(
        ...
        [session.tenant_id, body.account_name, body.account_number || '', body.bank_name || '', body.currency || 'KES', Number(body.opening_balance) || 0]
      );
    });
  ...
  try {
    ...
    await withTenantContext(session.tenant_id!, async () => {
      await run('UPDATE bank_accounts SET account_name=$1, account_number=$2, bank_name=$3, currency=$4, opening_balance=$5, is_active=$6 WHERE id=$7',
        [body.account_name, body.account_number || '', body.bank_name || '', body.currency || 'KES', Number(body.opening_balance) || 0, body.is_active ?? 1, body.id]);
    });
