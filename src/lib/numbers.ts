...
export async function generateNextNumber(type: 'invoice' | 'quotation' | 'credit_note'): Promise<string> {
  if (type === 'credit_note') {
    const settings = await get('SELECT credit_note_prefix, next_credit_note_number, last_credit_note_month FROM company_settings WHERE id = 1') as any;
    ...
    if (lastMonth !== currentMonth) {
      nextSeq = 1;
      await run(`UPDATE company_settings SET last_credit_note_month = $1, next_credit_note_number = $2 WHERE id = 1`, [currentMonth, nextSeq + 1]);
    } else {
      await run(`UPDATE company_settings SET next_credit_note_number = $1 WHERE id = 1`, [nextSeq + 1]);
    }
  ...
  if (lastMonth !== currentMonth) {
    nextSeq = 1;
    await run(`UPDATE company_settings SET ${lastMonthKey} = $1, ${nextKey} = $2 WHERE id = 1`, [currentMonth, nextSeq + 1]);
  } else {
    nextSeq = seq;
    await run(`UPDATE company_settings SET ${nextKey} = $1 WHERE id = 1`, [nextSeq + 1]);
  }
