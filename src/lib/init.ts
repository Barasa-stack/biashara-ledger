...
export async function ensureDbInitialized() {
  ...
  initPromise = (async () => {
    ...
    const migrated = tablesExist && await columnExists('users', 'license_key');

    if (!migrated && tablesExist) {
      const migrateCols = [
    ...
    if (tablesExist && !(await columnExists('company_settings', 'theme_color'))) {
      const cols = [
        `ALTER TABLE public.company_settings ADD COLUMN IF NOT EXISTS theme_color TEXT DEFAULT '#df1c1c'`,
        `ALTER TABLE public.company_settings ADD COLUMN IF NOT EXISTS invoice_footer_text TEXT DEFAULT ''`,
        `ALTER TABLE public.company_settings ADD COLUMN IF NOT EXISTS payment_instructions TEXT DEFAULT ''`,
    ...
    // Phase 3: ensure unique constraint on admin_license_keys.license_key for ON CONFLICT
    try {
      await exec(`
        DO $$
        BEGIN
    ...
    const allIndexes = [
      'idx_users_tenant_id', 'idx_sessions_tenant_id', 'idx_sessions_token',
      'idx_subscription_events_tenant_id', 'idx_billing_history_tenant_id',
      'idx_customers_tenant_id', 'idx_customers_name',
      'idx_clients_tenant_id', 'idx_clients_name',
      'idx_quotations_tenant_id', 'idx_quotations_customer_id',
      'idx_sales_invoices_tenant_id', 'idx_sales_invoices_customer_id',
      'idx_payments_tenant_id', 'idx_payments_invoice_id',
      'idx_credit_notes_tenant_id',
      'idx_purchase_orders_tenant_id', 'idx_purchase_invoices_tenant_id',
      'idx_supplier_payments_tenant_id', 'idx_supplier_payments_invoice_id',
